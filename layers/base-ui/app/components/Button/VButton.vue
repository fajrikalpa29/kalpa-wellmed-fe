...
<template>
  <component :is="as" :class="computedClass" :disabled="loading" v-bind="_.omit(attrs, 'class')">
    <span v-if="loading" class="flex items-center justify-center">Loading
      <LoadingIcon icon="three-dots" color="1a202c" className="ml-2" />
    </span>
    <template v-else>
      <slot></slot>
    </template>
