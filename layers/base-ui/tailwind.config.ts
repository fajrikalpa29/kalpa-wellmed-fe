...
export default {
  content: [
    join(currentDir, `./app/components/**/*.{js,vue,ts}`),
    join(currentDir, `./app/layouts/**/*.vue`),
    join(currentDir, `./app/pages/**/*.vue`),
    join(currentDir, `./app/nuxt.config.{js,ts}`),
    join(currentDir, `./app/app.vue`),
    join(currentDir, `./app/error.vue`),
    `../../core/app/pages/**/*.{js,ts,vue}`,
  ],
  darkMode: "class",
  theme: {
    container: {
    ...
    plugin(function ({ addBase, matchUtilities }: any) {
      addBase({
        ...
        ".theme-1": {
          ...
          "--color-info": toRGB("#42a5f5"),
          "--color-warning": toRGB("#f0b156"),
          "--color-pending": toRGB(colors.orange["700"]),
          "--color-danger": toRGB("#ff0b0b"),
          "--color-light": toRGB(colors.slate["100"]),
          "--color-dark": toRGB(colors.slate["800"]),
          "&.dark": {
            "--color-primary": toRGB(colors.sky["800"]),
